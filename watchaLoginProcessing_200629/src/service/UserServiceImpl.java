...
public class UserServiceImpl implements UserService {
	...
	public void logincheck(HttpServletRequest request, HttpServletResponse response) {
		...
		Userdb userdb = userdbDao.logincheck(userid);
		
		//System.out.println(userpw);
		//5. 결과를 가지고 필요한 처리를 수행
		if(userdb != null) {
			//비밀번호 확인
			if(BCrypt.checkpw(userpw, userdb.getUserpw())) {
				//비밀번호가 일치하는 경우 - 비밀번호는 삭제
				userdb.setUserpw(null);
			}else{
				//비밀번호가 틀린 경우 userdb == null
				userdb = null;
			}
		}
		JSONObject obj = new JSONObject();
		if(userdb != null) {
			obj.put("userid", userdb.getUserid());
			obj.put("usernickname", userdb.getUsernickname());
			obj.put("userprofileimage", userdb.getUserprofileimage());
		}else{
			obj = null;
		}
		...
		//6. 요청 처리 결과를 저장
		request.getSession().setAttribute("result", obj);
